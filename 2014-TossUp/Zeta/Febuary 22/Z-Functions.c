...
void liftlock (int speed, int time, int solenoidsArm, int solenoidsLock) {
	SensorValue[pneuLiftL] = SensorValue[pneuLiftR] = solenoidsArm;
	motor[liftL1] = motor[liftL2] = motor[liftR1] = motor[liftR2] = speed;
	wait1Msec(time);
	SensorValue[pneuLock] = solenoidsLock;
	motor[liftL1] = motor[liftL2] = motor[liftR1] = motor[liftR2] = 0;
}
...
void lifter(int arm)
{
  ...
  case medgoal:
    ...
    break;
  case lowgoal:
    while(SensorValue[pot]<((low + 100)-100))
    {
      motor[liftL1] = motor[liftL2] = 127;	//sets motors
			motor[liftR1] = motor[liftR2] = 127;
    }
