...
wait1Msec(500);          // pause
SensorValue[wing] = 0;
drive(-100,-70,1000);   // drive back to starting tile
collectorSet(0);
...
while(auton_flag == false && auton_flag2 == true) {            // loop for two auton cases
	...
	if(SensorValue[touchR] == 1) {        // case 1: scores in close goal
		collectorSet(127);
		drive(90,127,1150);
		wait1Msec(500);
		lifter(highgoal);
	...
	else if(SensorValue[touchL] == 1) {   // case 2: scores in far goal
